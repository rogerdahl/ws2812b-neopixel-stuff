...
// green and blue channels instead of reducing the overall brightness. If the combined value for the red, green and blue
// channels would be above the clamp value, the channels are adjusted so that their combined output is equal to the
// clamp value without altering the relative brightnesses of colors, which preserves the combined color.
//
// E.g., if one channel is fully on (value is 255) and the other two are fully off, clamping the light output to 255
// will allow the single channel to display at full brightness. Any combination of the three channels will get clamped
// to a combined value of 255, which is 1/3 of the possible combined value from the three channels. The difference is
// that if regular brightness is set to 1/3, the maximum possible value from any single channel is 256/3 = 85.
//
