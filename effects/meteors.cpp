...
//void loop()
//{
//  // runSec, delayMsec, numMeteors, minPixels, maxPixels, minSpeed, maxSpeed
//  // Overlapping, speed a function of size (smaller is faster)
//  meteors(60 * 60, 20, 6, 2, 25, 0, 30);
//  // Non-overlapping, evenly spaced, same speed
//  meteors(60 * 60, 20, 4, 15, 15, 20, 20);
//}
//
//void meteors(u16 runSec, u16 delayMsec, u8 numMeteors, u8 minPixels,
//             u8 maxPixels, s16 minSpeed, s16 maxSpeed)
//{
//  Segment segmentArr[numMeteors];
//  u16 numSuperPositions = (pixels.numPixels() + maxPixels) << 8;
//  u16 superPosOffset = numSuperPositions / numMeteors;
//
//  for (u8 i = 0; i < numMeteors; ++i) {
//    segmentArr[i].superPos = i * superPosOffset;
//    segmentArr[i].init = true;
//  }
//  u32 startMsec = millis();
//  while (millis() < startMsec + runSec * 1000UL) {
//    clear();
//    for (u8 i = 0; i < numMeteors; ++i) {
//      if (segmentArr[i].superPos >= numSuperPositions) {
//        segmentArr[i].superPos = 0;
//        segmentArr[i].init = true;
//      }
//      if (segmentArr[i].init) {
//        segmentArr[i].init = false;
//        segmentArr[i].color = random(0, 0xffffff);
//        segmentArr[i].numSegmentPixels = random(minPixels, maxPixels);
//        if (!minSpeed) {
//          segmentArr[i].speed = maxSpeed - segmentArr[i].numSegmentPixels;
//        }
//        else {
//          segmentArr[i].speed = random(minSpeed, maxSpeed);
//        }
//      }
//      drawTaperedSegment(segmentArr[i].superPos, segmentArr[i].numSegmentPixels,
